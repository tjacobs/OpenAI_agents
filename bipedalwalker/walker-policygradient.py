...
def policy_gradient():
    with tf.variable_scope("policy"):
        params = tf.get_variable("policy_parameters", [24, 4])  # Parameters
        state = tf.placeholder("float", [None, 24])             # World state
        actions = tf.placeholder("float", [None, 4])            # Actions
...
def value_gradient():
    with tf.variable_scope("value"):
        ...
        w1 = tf.get_variable("w1", [24, 10])             # Value gradient is *w1+b1, Relu, *w2+b2. 24, 10, 4.
        b1 = tf.get_variable("b1", [10])
        h1 = tf.nn.relu(tf.matmul(state, w1) + b1)
        w2 = tf.get_variable("w2", [10, 4])
...
env = gym.wrappers.Monitor(env, 'run-walker', force=True)
